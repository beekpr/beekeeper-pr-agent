...
class BeekeeperStyleGuidelinesFetcher:
    def __init__(
            self,
            repo_url='beekpr/beekeeper-engineering-hub',
            token=None,
            branch="master",
    ...
    def fetch_guidelines(self, force_refresh=False):
        ...
        for file_path, content in guidelines.items():
            logger.info(f"Found matches - {file_path}: {content['file_types']}")
        return guidelines
    ...
    def get_relevant_guidelines(self, file_paths):
        ...
        try:
            ...
            logger.error(f"Error fetching relevant guidelines: {str(e)}")
            return {}